...
每日北京时间 8:00 自动抓取数据、生成四联仪表盘并部署为公开静态站点。
...
- **国际政治日报** — 聚焦外交关系、多边机制、选举政局、冲突安全与政策治理
...
│   ├── fetch_politics.py      # 国际政治 RSS 抓取 + 分类 + 渲染
│   └── build_index.py         # 首页生成器（入口卡片 + 历史归档表）
└── site/                      # 构建产物（自动生成，部署目标）
    ├── index.html             # 首页
    ├── ai/index.html          # AI 仪表盘（当前期）
    ├── finance/index.html     # 金融仪表盘（当前期）
    ├── affairs/index.html     # 要闻仪表盘（当前期）
    ├── politics/index.html    # 政治仪表盘（当前期）
    ├── data/                  # 各仪表盘 meta.json
    └── archive/
...
- 执行：依次跑四个抓取脚本 → 生成首页 → `git commit & push` 回主分支
...
3. 构建配置：
   ...
   - **Build command**: `cd scripts && python fetch_ai.py && python fetch_finance.py && python fetch_affairs.py && python fetch_politics.py && python build_index.py`
...
金融、要闻和政治仪表盘使用 RSS 订阅源，部分通过 [RSSHub](https://docs.rsshub.app) 公共实例 `rsshub.app` 聚合。
...
3. **添加更多源**：在 `fetch_finance.py` / `fetch_affairs.py` / `fetch_politics.py` 的 `FEEDS` 列表里追加 `(url, source_name)` 元组即可
...
- 政治：`fetch_politics.py` 的 `CLASSIFY_RULES`
